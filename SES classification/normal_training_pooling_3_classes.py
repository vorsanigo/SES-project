...
# MAX NUMBER OF TWEETS TO USE PER USER
MAX_TWEETS = 32
...
class CamembertOrdinalPoolingFeatures(nn.Module):
    """
    Ordinal regression model for SES prediction:
        lower < medium < high

    CORAL formulation:
        3 classes -> 2 ordinal decisions

    Output logits:
        logit[0] : is class > lower ?
        logit[1] : is class > medium ?
    ...
    def forward(
        ...
        # detect valid tweets before reshape
        tweet_mask = (attention_mask.sum(dim=2) > 0).float()
        ...
        # attention pooling over tweets
        scores = self.attention(tweet_emb).squeeze(-1)

        # ignore padded tweets
        scores = scores.masked_fill(tweet_mask == 0, -1e9)
    ...
    def predict_classes(self, logits):
        """
        Convert ordinal logits -> final class ids
        """
...
def compute_metrics_ordinal(eval_pred):
    ...
    # threshold ordinal outputs
    preds = (ordinal_probs > 0.5).sum(dim=1).numpy()
...
# 1) TRAIN
def train(with_features, avg_type, n_features, ordinal, training_args, train_dataset, dev_dataset, tokenizer, SAVE_MODEL_DIR):
    ...
    if ordinal == False:
        ...
        model = CamembertOrdinalPoolingFeatures(n_features)
        print('ordinal')
...
def test(with_features, avg_type, feature_cols, n_features, NORM_FEATURES, training_args, TEST_FINAL_SES_ERRORS_PATH, SAVE_MODEL_DIR, SAVE_PREDICTIONS_PATH, num_tweets):
    ...
    test_df_grouped["prob_2"] = probs[:, 2]
    ...
    test_df_grouped[['user_id', 'label', 'prob_0', 'prob_1', 'prob_2']].to_csv(SAVE_PREDICTIONS_PATH, index=False)


def test_ordinal(with_features, avg_type, feature_cols, n_features, NORM_FEATURES, training_args, TEST_FINAL_SES_ERRORS_PATH, SAVE_MODEL_DIR, SAVE_PREDICTIONS_PATH, num_tweets):
    ...
    # binary ordinal decisions
    #
    # lower  -> [0,0]
    # medium -> [1,0]
    # high   -> [1,1]
    #
    binary_preds = (ordinal_probs > 0.5).int()

    # final class prediction
    preds = binary_preds.sum(dim=1).numpy()

    # optional: convert ordinal outputs into
    ...
    test_df_grouped["prob_0"] = prob_lower
    test_df_grouped["prob_1"] = prob_medium
    test_df_grouped["prob_2"] = prob_high
...
def evaluation(class_num, col_pred, TEST_PATH, PRED_PATH):
    ...
    # compute F1 score
    if class_num == 2:
        f1_score_num = f1_score(y_tot['SES_users_true'], y_tot['SES_users_pred'], average='macro') #compute_f1(y_tot['SES_users_true'], y_tot['SES_users_pred'])
    elif class_num == 3:
...
if __name__ == "__main__":
    ...
    # avg typ -> mean | attention | ordinal
    avg_type = 'attention'
    ...
    for features in FEATURES:
        ...
        '''if features in ['corr_features_negation', 'corr_features_with_length_negation']:
                continue'''
        ...
        for i in range(num_runs):
            ...
            if num_runs == 1:
                ...
                # output paths
                CHECKPOINTS_DIR = './checkpoints_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_3/' + features + scaling_name + '_' + avg_type
                SAVE_MODEL_DIR = './models_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_3/' + features + scaling_name + '_' + avg_type
                SAVE_PREDICTIONS_PATH = './predictions_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_3/' + features + scaling_name + '_' + avg_type + '.csv'

                # evaluation paths
                PRED_PATH = 'predictions_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_3/' + features + scaling_name + '_' + avg_type + '.csv'#predictions_normal_training_features_corr_NEW.csv
                OUTPUT_PATH = 'F1_score_dev_normal_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_3/' + features + scaling_name + '_' + avg_type + '_users.csv'#predictions_normal_training_features_corr_NEW.tsv
                ...
                if masked_location == True:
                    ...
                    CHECKPOINTS_DIR = f'./checkpoints_pre_avg_v2_3_classes_ILE_DE_FRANCE_MASKED_LOC/classes_{classes}/{features}{scaling_name}_{avg_type}/run_{i+1}'
                    
                    SAVE_MODEL_DIR = f'./models_pre_avg_v2_3_classes_ILE_DE_FRANCE_MASKED_LOC/classes_{classes}/{features}{scaling_name}_{avg_type}/run_{i+1}'
                    os.makedirs(SAVE_MODEL_DIR, exist_ok=True)
                    print('SAVE_MODEL_DIR', SAVE_MODEL_DIR)

                else:
                
                    CHECKPOINTS_DIR = f'./checkpoints_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_{classes}/{features}{scaling_name}_{avg_type}/run_{i+1}'
                    
                    SAVE_MODEL_DIR = f'./models_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_{classes}/{features}{scaling_name}_{avg_type}/run_{i+1}'
                    os.makedirs(SAVE_MODEL_DIR, exist_ok=True)
                    print('SAVE_MODEL_DIR', SAVE_MODEL_DIR)

                '''checkpoint = torch.load(f"{SAVE_MODEL_DIR}/model.pt")

                # check the classifier layer shapes in the saved file
                for k, v in checkpoint.items():
                    if 'classifier' in k:
                        print(k, v.shape)

                # and check all keys to make sure it's the right model
            ...
            # 2) TRAIN
            train(with_features, avg_type, N_FEATURES, ordinal, training_args, train_dataset, dev_dataset, tokenizer, SAVE_MODEL_DIR)
            ...
            for NUM_TWEETS in NUM_TWEETS_LIST:
                ...
                if masked_location == True:
                    ...
                    # output eval
                    SAVE_DIR_EVAL = f'F1_score_dev_normal_pre_avg_v2_3_classes_ILE_DE_FRANCE_MASKED_LOC/classes_{classes}'
                    os.makedirs(SAVE_DIR_EVAL, exist_ok=True)
                    OUTPUT_PATH_EVAL = f'{SAVE_DIR_EVAL }/{features}{scaling_name}_{avg_type}.csv'

                    SAVE_PREDICTIONS_DIR = f'./predictions_pre_avg_v2_3_classes_ILE_DE_FRANCE_MASKED_LOC/classes_{classes}_{NUM_TWEETS}/{features}{scaling_name}_{avg_type}/run_{i+1}'
                    ...
                    # output eval
                    SAVE_DIR_EVAL = f'F1_score_dev_normal_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_{classes}'
                    os.makedirs(SAVE_DIR_EVAL, exist_ok=True)
                    OUTPUT_PATH_EVAL = f'{SAVE_DIR_EVAL }/{features}{scaling_name}_{avg_type}.csv'

                    SAVE_PREDICTIONS_DIR = f'./predictions_pre_avg_v2_3_classes_ILE_DE_FRANCE/classes_{classes}_{NUM_TWEETS}/{features}{scaling_name}_{avg_type}/run_{i+1}'
                    os.makedirs(SAVE_PREDICTIONS_DIR, exist_ok=True)
                    SAVE_PREDICTIONS_PATH = f'{SAVE_PREDICTIONS_DIR}/{features}{scaling_name}_{avg_type}.csv'
                ...
                if ordinal == False:
                    ...
                    test_ordinal(with_features, avg_type, feature_cols, N_FEATURES, NORM_FEATURES, test_args, TEST_FINAL_SES_ERRORS_PATH, SAVE_MODEL_DIR, SAVE_PREDICTIONS_PATH, NUM_TWEETS)
